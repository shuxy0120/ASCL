...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='../data', dataset='cpv2',
        ...
        for i in range(len(self.entries)):  # 438183
            self.entries[i]['question_label'] = i*2
    ...
    def __getitem__(self, index):
        ...
        if self.image_to_fe is not None:
            ...
            features_dic = torch.load('../data/rcnn/' + str(entry["image_id"]) + '.pth')
            # features_dic = torch.load('../data/rcnn-test2015/' + str(entry["image_id"]) + '.pth')
            features = torch.from_numpy(features_dic['image_features'])
            bbox = torch.from_numpy(features_dic['image_bb'])
        ...
        img_h, img_w = self.hw[str(entry["image_id"])]['image_h'], self.hw[str(entry["image_id"])]['image_w']
        bbox[:, (0, 2)] /= img_w
        bbox[:, (1, 3)] /= img_h
        
        # np.testing.assert_array_less(bbox, 1+1e-5)
        # np.testing.assert_array_less(-bbox, 0+1e-5)
        ...
        if labels is not None:
            ...
            target_mask = torch.cat((torch.ones(labels.size(0)), torch.zeros(10 - labels.size(0))))
        else:
            target_mask = torch.zeros(10)
        ...
        if self.name=='train':
            ...
            if "bias" in entry:
                return features, raw_question, target, entry["bias"], bbox, q_id, target_mask, train_hint, entry['question_label']

            else:
                return features, raw_question,target, bbox, q_id
        else:
            test_hint=torch.tensor(self.test_hintsocre[str(q_id)])
            if "bias" in entry:
                return features, raw_question, target, entry["bias"], bbox, q_id, target_mask, test_hint, entry['question_label']
            else:
                return features, raw_question, target, bbox, q_id
