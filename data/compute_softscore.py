...
def load_cp():
    train_answer_file = "vqacp_v2_train_annotations.json"
    with open(train_answer_file) as f:
        train_answers = json.load(f)  # ['annotations']

    val_answer_file = "vqacp_v2_test_annotations.json"
    with open(val_answer_file) as f:
        val_answers = json.load(f)  # ['annotations']

    occurence = filter_answers(train_answers, 9)
    ans2label = create_ans2label(occurence, 'trainval', "cp-cache")
    compute_target(train_answers, ans2label, 'train', "cp-cache")
    compute_target(val_answers, ans2label, 'val', "cp-cache")

def load_cp_v1():
    train_answer_file = "vqacp_v1_train_annotations.json"
    with open(train_answer_file) as f:
        train_answers = json.load(f)  # ['annotations']

    val_answer_file = "vqacp_v1_test_annotations.json"
    with open(val_answer_file) as f:
        val_answers = json.load(f)  # ['annotations']

    occurence = filter_answers(train_answers, 9)
    ans2label = create_ans2label(occurence, 'trainval', "cp-v1-cache")
    compute_target(train_answers, ans2label, 'train', "cp-v1-cache")
    compute_target(val_answers, ans2label, 'val', "cp-v1-cache")
