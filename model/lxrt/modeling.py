...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.x_layers = nn.ModuleList(
            [LXRTXLayer(config) for _ in range(5)]  # self.num_x_layers
        )
        self.r_layers = nn.ModuleList(
            [BertLayer(config) for _ in range(self.num_r_layers)]
        )
        config.hidden_act="relu"
        self.x_layers2 = nn.ModuleList(
            [LXRTXLayer(config) for _ in range(2)]  # self.num_x_layers
        )

        self.cl_loss = lossfunc.ContrastiveLoss(margin=0.5).cuda()
        self.v_gum_t = torch.nn.Linear(768, 1)


    def forward(self, lang_feats, lang_attention_mask,
                visn_feats, visn_attention_mask=None, qid=None, mode='train', q_index=None):
        ...
        batch_size, samples, hidden_dim = visn_feats.size()
        q_out = (lang_feats).mean(1).unsqueeze(1)
        vq_att = torch.matmul(visn_feats/ (torch.norm(visn_feats, 2, -1, keepdim=True).expand_as(visn_feats) + 1e-12), (q_out/ (torch.norm(q_out, 2, -1, keepdim=True).expand_as(q_out) + 1e-12)).permute(0, 2, 1)).squeeze(-1)

        vq_att_sort, sort_idx = torch.sort(vq_att, descending=True, dim=1)
        _, sort_idx2 = torch.sort(sort_idx, dim=1)
        v_mask = []
        v_gumbel_t = F.relu(self.v_gum_t(visn_feats)).squeeze(-1)
        v_sort = v_gumbel_t.gather(1, sort_idx)
        for i in range(batch_size):
            mask_index = torch.nn.functional.gumbel_softmax(v_sort[i], tau=1, hard=True)
            temp = [i + 1 for i in range(0, samples - 1)]
            temp.append(samples - 1)
            index_space = torch.LongTensor(temp).cuda()
            index = (index_space * mask_index).sum(-1)
            mask = torch.cat(
                (torch.ones(int(index.item())), torch.zeros(samples - int(index.item())))).cuda()
            mask[mask_index.argmax().item()] -= 1
            mask = mask_index + mask
            v_mask_k = mask.gather(0, sort_idx2[i])
            v_mask.append(v_mask_k.unsqueeze(0))
        v_mask = torch.cat(v_mask, dim=0)
        v_pos = visn_feats * v_mask.unsqueeze(-1)
        v_neg = visn_feats * (1-v_mask).unsqueeze(-1)

        if mode == 'train':
            pn_loss = list()
            for i in range(batch_size):
                c = ((vq_att[i] - vq_att[i].min()) / (vq_att[i].max() - vq_att[i].min()))
                c = c.unsqueeze(0).expand(36, 36)
                c = torch.where(c>c.T, c, c.T)
                c = (c.mean()-c)/4. + 0.5
                pn_loss.append(self.cl_loss(visn_feats[i], v_mask[i], c))
            pn_loss = sum(pn_loss) / batch_size

        lang_feats_neg = lang_feats.clone()
        visn_feats = v_pos
        visn_attention_mask = v_mask
        ...
        for layer_module in self.x_layers2:
            lang_feats, visn_feats = layer_module(lang_feats, lang_attention_mask,
                                                  visn_feats, visn_attention_mask)

        if mode == 'train':
            v_neg_attention_mask = 1-v_mask
            v_neg_attention_mask = v_neg_attention_mask.unsqueeze(1).unsqueeze(2)
            v_neg_attention_mask = v_neg_attention_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
            v_neg_attention_mask = (1.0 - v_neg_attention_mask) * -10000.0
            for layer_module in self.x_layers2:
                lang_feats_neg, v_neg = layer_module(lang_feats_neg, lang_attention_mask,
                                                      v_neg, v_neg_attention_mask)
        else:
            lang_feats_neg = None
            pn_loss = None
        return lang_feats, visn_feats, pn_loss, lang_feats_neg
...
class BertPooler2(nn.Module):
    ...
    def forward(self, hidden_states, q_index):
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token.
        first_token_tensor = hidden_states[torch.arange(hidden_states.size(0)), q_index]
        pooled_output = self.dense(first_token_tensor)
        pooled_output = self.activation(pooled_output)
        return pooled_output
...
class LXRTModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                visual_feats=None, visual_attention_mask=None, qid=None, mode='train'):
        ...
        q_index=input_ids.argmax(-1)
        
        # Run LXRT backbone
        lang_feats, visn_feats, pn_loss, lang_feats_neg = self.encoder(
            embedding_output,
        ...
        if mode == 'train' and lang_feats_neg is not None:
            pooled_output_neg = self.pooler(lang_feats_neg)
        else:
            pooled_output_neg = None

        return (lang_feats, visn_feats), pooled_output, pn_loss, pooled_output_neg
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,
                visual_attention_mask=None, qid=None, mode='train'):
        feat_seq, pooled_output, pn_loss, pooled_output_neg = self.bert(input_ids, token_type_ids, attention_mask,
                                                     visual_feats=visual_feats,
                                                     visual_attention_mask=visual_attention_mask,
                                                     qid=qid,
                                                     mode=mode)
        if 'x' == self.mode:
            return pooled_output, pn_loss, pooled_output_neg
